...
class BookingApplicationTests {
  ...
  void updateEventTest() {
    ...
    Event eventToUpdate =
        Event.builder().id(1L).title("New Event").date(Date.valueOf("2020-12-12")).build();
  ...
  void updateNonExistingEventTest() {
    Event eventToUpdate =
        Event.builder().id(11L).title("New Event").date(Date.valueOf("2020-12-12")).build();
    Assertions.assertThrows(
        NonExistentEventException.class, () -> bookingService.updateEvent(eventToUpdate));
  }
  ...
  void updateUserTest() {
    ...
    User userToUpdate = User.builder().id(1L).name("Juan").email("juan@epam.com").build();
  ...
  void updateNonExistingUserTest() {
    User userToUpdate = User.builder().id(11L).name("Laura").email("laura@epam.com").build();
    Assertions.assertThrows(
        NonExistentUserException.class, () -> bookingService.updateUser(userToUpdate));
  }
  ...
  @Test
  void bookTicketTest() {
    User userToBook = User.builder().id(1L).name("Laura").email("laura@epam.com").build();
    ...
    Ticket ticketBooked =
        bookingService.bookTicket(userToBook.getId(), eventToBook.getId(), place, CONFERENCES);
  ...
  void getBookedTicketsByUserTest() {
    User user = User.builder().id(1L).name("Laura").email("laura@epam.com").build();
